...
namespace Microsoft.BotBuilderSamples.Dialogs
{
    public class MainDialog : ComponentDialog
    {
        ...
        private async Task<DialogTurnResult> IntroStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            var messageText = stepContext.Options?.ToString() ?? $"Hola, deseas reservar una cita? ";
        ...
        private async Task<DialogTurnResult> ActStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            {
                case FlightBooking.Intent.GenerarConsulta:
                    // Initialize BookingDetails with any entities we may have found in the response.
                    var bookingDetails = new BookingDetails()
                    {
                        Equipo = cluResult.Entities.GetEquipo(),
                        //Origin = cluResult.Entities.GetFromCity(),
                        Marca = cluResult.Entities.GetMarca(),
                    };
                ...
                case FlightBooking.Intent.GenerarReclamo:
        ...
        private async Task<DialogTurnResult> FinalStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (stepContext.Result is BookingDetails result)
            {
                ...
                //var timeProperty = new TimexProperty(result.Marca);
               // var travelDateMsg = timeProperty.ToNaturalLanguage(DateTime.Now);
                var messageText = $"Se reservará su cita para comprar una computadora Lenovo{result.Equipo}"; //on { travelDateMsg}
                var message = MessageFactory.Text(messageText, messageText, InputHints.IgnoringInput);
                await stepContext.Context.SendActivityAsync(message, cancellationToken);
            }

            // Restart the main dialog with a different message the second time around
            var promptMessage = "Su cita se agendó con éxito";
